...
{ 
  ...
  analysisManager->CreateH1("posX","Energy deposited along x", 100, -14.*cm, 14.*cm);
  analysisManager->CreateH1("posY","Energy deposited along y", 200, -12.*cm, 12.*cm);
  analysisManager->CreateH1("posZ","Energy deposited along z", 120, -14.0*cm, 14.0*cm);
  analysisManager->CreateH1("radius","radius", 140, -10.0*cm, 10.0*cm);
  // 2D histograms
  analysisManager->CreateH2("xy","xy", 100, -140., 140., 100, -120., 120.);
  analysisManager->CreateH2("yz","yz", 100, -120., 120., 100, -160., 160.);
  analysisManager->CreateH2("zx","zx", 100, -160., 160., 100, -140., 140.);
